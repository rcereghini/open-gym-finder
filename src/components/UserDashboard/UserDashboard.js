			...
			<div className="user-dashboard-main">
				...
				{this.state.addNewGymVisible && !this.state.addNewGymConfirm ? (
					<div className="gym-add-confirm" style={{ height: "100%" }}>
						<h2>NOTICE: Your gym must be verified before listing open mats. This may take up to 48 hours.</h2>
						<button onClick={this.addNewGymConfirm}>Continue</button>
					</div>
				) : null}
				{this.state.addNewGymVisible && this.state.addNewGymConfirm ? <AddGymForm /> : null}
				<div
				...
				{!this.state.addNewGymVisible && !this.state.findGymVisible ? (
					...
					>
						<h1 style={{ color: "white", fontSize: "14px" }}>Additional stats to go here.</h1>
					</div>
				) : null}
				{/* {!this.state.addNewGymVisible && !this.state.findGymVisible ? (
          <Settings />
